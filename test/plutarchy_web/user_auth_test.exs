defmodule PlutarchyWeb.UserAuthTest do
  use PlutarchyWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Plutarchy.Accounts
  alias Plutarchy.Accounts.Scope
  alias PlutarchyWeb.UserAuth

  import Plutarchy.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PlutarchyWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PlutarchyWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PlutarchyWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
